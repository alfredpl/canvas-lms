...
set :repository,    "https://github.com/alfredpl/canvas-lms.git"
set :scm,     :git
...
#set me for future
set :ping_url, "https://198.199.113.195/login"
...
namespace :canvas do
  ...
  task :copy_config do
    folder = 'tmp/files'
    run "ln -nfs #{lms_data_files} #{latest_release}/#{folder}"
    ...
    run "ln -nfs #{shared_path}/fix/enter_key_fix.js #{release_path}/public/javascripts/translations/enter_key_fix.js"
  end
  ...
  task :canvasuser_permission do
	...
	sudo "chown -R #{current_path}/public/stylesheets/compiled" 
	sudo "chown -R #{current_path}/Gemfile.lock"
	sudo "chown -R#{current_path}/config.ru"
  end
